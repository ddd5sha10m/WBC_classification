...
# -*- coding: utf-8 -*-
"""
EfficientNet Fine-Tune for WBC Classification
- PyTorch + torchvision
- Pretrained EfficientNet-B0
- Two-phase training: head-only then full fine-tune
- Augmentations: RandomResizedCrop, RandAugment, flips, rotation
- MixUp (optionally CutMix)
- Optimizer: AdamW (default) or SGD
- Scheduler: OneCycleLR
- Class-balanced options (WeightedRandomSampler or class weights)
- Grad-CAM utility for sanity-checking focus regions
- Minimal, production-ready structure with clear hooks/params

Usage (example):
    python efficientnet_finetune_wbc.py \
        --data_root /path/to/dataset_root \
        --train_dir train --val_dir val \
        --num_classes 8 \
        --epochs_head 8 --epochs_ft 40 \
        --batch_size 32 \
...
- Directory structure:
    ...
    data_root/val/<class>/*.jpg|png
- Images can be RGB; non-RGB will be auto-converted.

Tip:
- Start with default params, then edit hyperparams and rerun.
"""
